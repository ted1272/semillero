...
cajacajero_bp = Blueprint('cajacajero', __name__)
...
@cajacajero_bp.route('/cajacajero', methods=['GET'])
@proteger_ruta
def cajacajero():
    cur = mydb.cursor()
    ...
    render_result = render_template('cajero/cajacajero.html', productos=productos, clientes=clientes, total_carrito=total_carrito, carrito=carrito)

    return render_result

@cajacajero_bp.route('/clienteht_cajero', methods=['POST'])
@proteger_ruta
def clienteht():
    ...
    return redirect(url_for('cajacajero.cajacajero'))


@cajacajero_bp.route('/agregar_cajacajero', methods=['POST'])
@proteger_ruta
def agregar_cajacajero():
    try:
        if request.method == 'POST':
        ...
        print(f"Error en la ruta /agregar_cajacajero: {str(e)}")
...
@cajacajero_bp.route('/mostrarsuma', methods=['POST'])
@proteger_ruta
def suma():
    cliente_id = session.get('cliente_id')
    cur = mydb.cursor()
...
@cajacajero_bp.route('/eliminar_carrito/<int:idcarrito>')
def eliminar_carrito(idcarrito):
    try:
        ...
        mydb.commit()
        cur.close()
        return redirect(url_for('cajero/cajacajero.cajacajero'))  
    except Exception as ex:
        return jsonify({'mensaje': f"Error: {str(ex)}"}), 500



@cajacajero_bp.route('/mostrarcarrito', methods=['POST'])
@proteger_ruta
def mostrarcarrito():
    cliente_id = session.get('cliente_id')
    cur = mydb.cursor()
...
@cajacajero_bp.route('/convertir_venta_cajacajero', methods=['POST'])
@proteger_ruta
def convertir_venta_cajacajero():
    cliente_id = session.get('cliente_id')
    ...
    return render_template('cajero/cajacajero.html')
